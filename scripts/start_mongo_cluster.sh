...
then
	...
	then
		...
		docker container run -d -p $PORT:$PORT --name $AWS_INSTANCE_NAME mongo --port $PORT --replSet s0 --shardsvr -smallfiles
	...
	then
		...
		docker container run -d -p $PORT:$PORT --name $AWS_INSTANCE_NAME mongo --port $PORT --replSet s1 --shardsvr -smallfiles
	...
	then
		...
		docker container run -d -p $PORT:$PORT --name $AWS_INSTANCE_NAME mongo --port $PORT --replSet conf --configsvr -smallfiles
	else
		echo "#################################"
